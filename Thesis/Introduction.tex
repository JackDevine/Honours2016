...
\end{align}
If there are no particles are flowing through the boundaries and if there is no heat flowing through the boundaries, then there is no exchange of energy with the external environment and the first and last terms vanish. If we let $\kappa = \frac{1}{c_p}$, then the middle terms cancel and the first law is obeyed by the system.

As for the entropy, we have \cite{Streater1997a}
\begin{equation}
S(t) = -k_B \int P(x, t) \log(P(x, t)) dx + c_p \int \log(T(x, t))dx
\end{equation}
...
\begin{align}
\frac{d S}{d t} =  k_B \int \frac{\partial J}{\partial x} + \frac{\partial J}{\partial x} \log P \ dx + c_p \int \frac{1}{T} \left(-\kappa J \partial_x V + c D \frac{\partial^2 T}{\partial x^2} \right) dx \\
                     = k_B \left ( [J \log P]_{\partial \Omega} - \int_{\partial \Omega} \frac{J}{P} \frac{\partial P}{\partial x} dx + [J]_{\partial \Omega} \right) - \int_{\partial \Omega} \frac{J}{T} \frac{\partial V}{\partial x} + c_p D \int_{\partial \Omega} \frac{1}{T} \frac{\partial^2 T}{\partial x^2} dx
\end{align}

We will denote the boundary terms with $B(t) = k_B( [J \log P]_{\partial \Omega} + [\frac{\partial J}{\partial x}]_{\partial \Omega} ) $, now the change in entropy becomes:

\begin{eqnarray}
\frac{d S}{d t} & = & - \int_{\partial_\Omega} \frac{J}{P} \frac{\partial P}{\partial x} + \frac{J}{T} \frac{\partial V}{\partial x} dx +  c D \int_{\partial_\Omega} \frac{1}{T} \frac{\partial^2 T}{\partial x^2} dx + B(t) \\
                    & = & \int_{\partial_\Omega} \frac{J^2}{T P} dx + c D \int_{\partial_\Omega} \frac{1}{T} \frac{\partial^2 T}{\partial x^2} dx + B(t)
\end{eqnarray}
